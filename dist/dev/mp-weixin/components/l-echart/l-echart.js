...
const components_lEchart_canvas = require("./canvas.js");
const components_lEchart_utils = require("./utils.js");
const _sfc_main = {
  name: "lime-echart",
  props: {
    type: {
  ...
  created() {
    ...
    this.use2dCanvas = this.type === "2d" && components_lEchart_utils.canIUseCanvas2d();
  },
  mounted() {
    this.$nextTick(() => {
      this.$emit("finished");
  ...
  methods: {
    ...
    async init(echarts, ...args) {
      ...
      if (this.beforeDelay) {
        await components_lEchart_utils.sleep(this.beforeDelay);
      }
      let config = await this.getContext();
      components_lEchart_canvas.setCanvasCreator(echarts, config);
    ...
    getContext() {
      return components_lEchart_utils.getRect(`#${this.canvasId}`, { context: this, type: this.use2dCanvas ? "fields" : "boundingClientRect" }).then((res) => {
        if (res) {
          let dpr = components_lEchart_utils.devicePixelRatio;
          ...
          if (node) {
            ...
            canvas = new components_lEchart_canvas.Canvas(ctx, this, true, node);
            this.canvasNode = node;
          } else {
            dpr = this.isPC ? components_lEchart_utils.devicePixelRatio : 1;
            this.rect = res;
            this.nodeWidth = width * dpr;
            this.nodeHeight = height * dpr;
            const ctx = common_vendor.index.createCanvasContext(this.canvasId, this);
            canvas = new components_lEchart_canvas.Canvas(ctx, this, false);
          }
    ...
    touchStart(e) {
      this.isDown = true;
      const next = () => {
        const touches = components_lEchart_utils.convertTouchesToArray(e.touches);
        if (this.chart) {
          ...
          components_lEchart_canvas.dispatch.call(handler, "mousedown", touch);
          components_lEchart_canvas.dispatch.call(handler, "mousemove", touch);
          handler.processGesture(components_lEchart_utils.wrapTouch(e), "start");
          clearTimeout(this.endTimer);
        }
      };
      if (this.isPC) {
        components_lEchart_utils.getRect(`#${this.canvasId}`, { context: this }).then((res) => {
          this.rect = res;
          next();
        });
        return;
    ...
    touchMove(e) {
      ...
      if (this.chart && this.isDown) {
        const handler = this.chart.getZr().handler;
        components_lEchart_canvas.dispatch.call(handler, "mousemove", this.getTouch(e, touches));
        handler.processGesture(components_lEchart_utils.wrapTouch(e), "change");
      }
    },
    touchEnd(e) {
      this.isDown = false;
      if (this.chart) {
        const touches = components_lEchart_utils.convertTouchesToArray(e.changedTouches);
        const { x } = touches && touches[0] || {};
        const touch = (x ? touches[0] : this.getRelative(e, touches)) || {};
        const handler = this.chart.getZr().handler;
        const isClick = Math.abs(touch.x - this.startX) < 10 && new Date() - this.startT < 200;
        components_lEchart_canvas.dispatch.call(handler, "mouseup", touch);
        handler.processGesture(components_lEchart_utils.wrapTouch(e), "end");
        if (isClick) {
          components_lEchart_canvas.dispatch.call(handler, "click", touch);
        } else {
          this.endTimer = setTimeout(() => {
            components_lEchart_canvas.dispatch.call(handler, "mousemove", { x: 999999999, y: 999999999 });
            components_lEchart_canvas.dispatch.call(handler, "mouseup", { x: 999999999, y: 999999999 });
          }, 50);
        }
      }
    }
...
const Component = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__file", "E:/vueCode/code1/okr-miniprogram/src/components/l-echart/l-echart.vue"]]);
