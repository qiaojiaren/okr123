...
const handlePhotoUrl = () => {
  for (let i = 0; i < arr.value.length; i++) {
    if (!arr.value[i].photo.startsWith("http://y4pqfi.natappfree.cc/")) {
      // 如果不是以指定前缀开头，则在字符串前拼接上指定前缀
      arr.value[i].photo = 'http://y4pqfi.natappfree.cc/' + arr.value[i].photo
    }
  }
}
    ...
    success: (res) => {
      ...
      if (res.confirm) {
        if (!loginMessage.priviledge)
          ...
          uni.request({
            url: `http://y4pqfi.natappfree.cc/teampersonal/remove/${id}`,
            method: 'get',
            header: {
              "Login-Type": "r6Vsr0",
              "Token": loginMessage.token
...
<template>
  <div class="mian">
    ...
    <uni-list :border="true">
      ...
      <img v-else class="empty" src="http://y4pqfi.natappfree.cc/media/static/空状态成员列表.png" alt="">
    </uni-list>
  </div>
</template>
